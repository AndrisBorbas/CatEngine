...
private:
	...
	void SetupVulkanWindow( ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height )
	{
		...
		wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat( physicalDevice_, wd->Surface, requestSurfaceImageFormat,
		...
		ImGui_ImplVulkanH_CreateOrResizeWindow(
			instance_, physicalDevice_, device_, wd, QueueFamily_, nullptr, width, height, 2 );
	}
	...
	void initImgui()
	{
		...
		wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat( physicalDevice_, wd->Surface,
			reinterpret_cast< const VkFormat* >( requestSurfaceImageFormat ), (size_t)IM_ARRAYSIZE( requestSurfaceImageFormat ),
			(VkColorSpaceKHR)requestSurfaceColorSpace );
	...
	void initVulkan()
	{
		...
		std::tie( depthImage_, depthImageMemory_, depthImageView_ ) =
			createDepthResources( device_, physicalDevice_, swapchainExtent_, graphicsQueue_, commandPool_, msaaSamples_ );
		createFramebuffers( device_, swapChainFramebuffers_, swapchainImageViews_, colorImageView_, depthImageView_,
			renderPass_, swapchainExtent_ );
		std::tie( textureImage_, textureImageMemory_ ) =
			createTextureImage( device_, physicalDevice_, "assets/textures/viking_room.png", STBI_rgb_alpha, mipLevels_,
				vk::Format::eR8G8B8A8Srgb, graphicsQueue_, commandPool_ );
		textureImageView_ =
			createImageView( device_, textureImage_, vk::Format::eR8G8B8A8Srgb, vk::ImageAspectFlagBits::eColor, mipLevels_ );
		textureSampler_ = createTextureSampler( device_, physicalDevice_, mipLevels_ );

		createVertexBuffer(
			device_, physicalDevice_, commandPool_, graphicsQueue_, vertices_, vertexBuffer_, vertexBufferMemory_ );
		createIndexBuffer( device_, physicalDevice_, commandPool_, graphicsQueue_, indices_, indexBuffer_, indexBufferMemory_ );
		createUniformBuffers( device_, physicalDevice_, uniformBuffers_, uniformBuffersMemory_, swapchainImages_ );
		descriptorPool_ = createDescriptorPool( device_, swapchainImages_ );
		createDescriptorSets( device_, descriptorSetLayout_, descriptorSets_, swapchainImages_, descriptorPool_,
			uniformBuffers_, textureImageView_, textureSampler_ );
		...
		createSyncObjects( device_, imageAvailableSemaphores_, renderFinishedSemaphores_, inFlightFences_, imagesInFlight_,
			MAX_FRAMES_IN_FLIGHT, swapchainImages_ );
		initImgui();
	}
	...
	void reloadModel( std::string filename )
	{
		...
		std::tie( depthImage_, depthImageMemory_, depthImageView_ ) =
			createDepthResources( device_, physicalDevice_, swapchainExtent_, graphicsQueue_, commandPool_, msaaSamples_ );
		std::tie( textureImage_, textureImageMemory_ ) =
			createTextureImage( device_, physicalDevice_, ( "assets/textures/" + filename + ".png" ).c_str(), STBI_rgb_alpha,
	...
	void drawFrame()
	{
		...
		commandBuffers_[imageIndex].bindDescriptorSets(
			vk::PipelineBindPoint::eGraphics, pipelineLayout_, 0, 1, &descriptorSets_[imageIndex], 0, nullptr );
